...
@can('school_class_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("dashboard.school-classes.create") }}">
                ...
                <tbody>
                    @foreach($schoolClasses as $key => $schoolClass)
                        <tr data-entry-id="{{ $schoolClass->id }}">
                            ...
                            <td>
                                <a href="{{ route('dashboard.calendar.index') }}?class_id={{ $schoolClass->id }}">View Schedule</a>
                            </td>
                            <td>
                                @can('school_class_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('dashboard.school-classes.show', $schoolClass->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('school_class_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('dashboard.school-classes.edit', $schoolClass->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('school_class_delete')
                                    <form action="{{ route('dashboard.school-classes.destroy', $schoolClass->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('school_class_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('dashboard.school-classes.massDestroy') }}",
